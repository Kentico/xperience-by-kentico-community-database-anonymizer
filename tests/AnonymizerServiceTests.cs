...
namespace XperienceCommunity.DatabaseAnonymizer.Tests
{
    ...
    public class AnonymizerServiceTests : ContainerNotBuiltUnitTests
    {
        ...
        [Test]
        public void Anonymize_AnonymizesSettingsKeys()
        {
            string key1 = "Key_1";
            string key2 = "Key_2";
            anonymizerService.Anonymize(new ConnectionSettings(), new()
            {
                Tables = [],
                SettingsKeys = [key1, key2]
            });

            string expectedQuery = $"UPDATE CMS_SettingsKey SET KeyValue = NULL WHERE KeyName IN ('{key1}','{key2}')";

            dataConnection.Received().ExecuteNonQuery(
                Arg.Is<string>(s => s.Equals(expectedQuery, StringComparison.OrdinalIgnoreCase)),
                null,
                QueryTypeEnum.SQLQuery,
                false);
        }
